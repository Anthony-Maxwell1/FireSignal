...
async function initMap() {
    try {
        ...
        socket.onopen = function (event) {
            ...
            if (urlParams.get('username') != null) {
                ...
                if (username == '' || password == '' || organization == '' || organization == null) {
                    ...
                    socket.send(JSON.stringify({'client': 'firefighter', 'type': 'sign-in', 'data': {'username': username, 'password': password, 'organization': organization}}));
                }
            } else {
                document.getElementById('sign-in').classList.remove('hidden')
            }
        ...
        socket.onerror = function (event) {
            ...
            document.getElementById(`loading`).innerHTML = `An Error has occurred.`;
        }
...
function reportfire() {
    socket.send(JSON.stringify({'client': 'firefighter', 'type': 'report', 'data': currPos, 'sessionID': sessionID}));
